...
impl LocationData {
    ...
    */
    #[cfg(target_arch = "wasm32")]
    fn get_current_location(state: Rc<RefCell<LocationData>>, ctx: eframe::egui::Context) {
    ...
    */
    ...
    fn make_success_callback(
        ...
        Closure::<dyn FnMut(web_sys::Position)>::new(move |position: web_sys::Position| {
            ...
            spawn_local(async move {
                match LocationData::reverse_geocode(latitude, longitude).await {
                    Ok(address) => {
                    ...
                    Err(err) => {
                        ...
                        // We still have coordinates and can search with them;
                        // we just couldn't turn them into a readable address.
