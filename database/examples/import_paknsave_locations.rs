...
fn main() {
    ...
    let json_text = fs::read_to_string("../data/paknsave_locations.json")
        .expect("could not read ../data/paknsave_locations.json — did you copy it there?");

    let parsed: Value = serde_json::from_str(&json_text)
        .expect("paknsave_locations.json was not valid JSON");
    ...
    let deleted = conn
        ...
        .expect("failed to clear old Pak'nSave locations");
    println!("Cleared {} existing Pak'nSave location rows.", deleted);
    ...
    println!("Inserted {} Pak'nSave store locations.", inserted);
    ...
    println!("Total Pak'nSave locations now in database: {}", total);
}