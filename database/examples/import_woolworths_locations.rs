...
fn main() {
    ...
    let json_text = fs::read_to_string("../data/woolworths_locations.json")
        .expect("could not read ../data/woolworths_locations.json — did you copy it there?");

    let parsed: Value = serde_json::from_str(&json_text)
        .expect("woolworths_locations.json was not valid JSON");
    ...
    let deleted = conn
        ...
        .expect("failed to clear old Woolworths locations");
    println!("Cleared {} existing Woolworths location rows.", deleted);
    ...
    println!("Inserted {} Woolworths store locations.", inserted);
    ...
    println!("Total Woolworths locations now in database: {}", total);
}